...
public class MessageManager {
    ...
    private final HashMap<Byte, Reaction> messageReactions;
    ...
    public MessageManager(HashMap<Byte, Reaction> messageReactions) {
        ...
        handshake.setProtocolName("Bittorrent Protocol".getBytes(StandardCharsets.US_ASCII));
    }
    ...
    public Handshake receiveHandshake(SocketChannel socket) throws IOException {
        ...
        if ((socket.read(lengthBuf)) != 1) throw new IOException("Unexpected EOF.");

        lengthBuf.rewind();
        protocolNameLength = lengthBuf.get();

        if (protocolNameLength != PROTOCOL_NAME_LENGTH) throw new IOException("Invalid clientHandshake.");

        protocolNameBuf = ByteBuffer.allocate(protocolNameLength);

        if (socket.read(protocolNameBuf) != protocolNameLength) throw new IOException("Invalid clientHandshake.");

        if (socket.read(reservedBytesBuf) != NUM_RESERVED_BYTES) throw new IOException("Invalid clientHandshake.");

        if (socket.read(hashBuf) != HASH_LENGTH) throw new IOException("Invalid clientHandshake.");

        if (socket.read(peerIdBuf) != PEER_ID_LENGTH) throw new IOException("Invalid clientHandshake.");
    ...
    public boolean isValidHandshake(Handshake clientHandshake, byte[] expectedPeerID) {
        if (Arrays.equals(expectedPeerID, clientHandshake.getPeerID())) return isValidHandshake(clientHandshake);

        return false;
    }

    public void sendMessage(SocketChannel channel, ByteBuffer message) throws IOException {
    ...
    public ByteBuffer generateNotInterested() {
        int length = ID_SIZE;
        ByteBuffer message = ByteBuffer.allocate(LENGTH_SIZE + length);
        byte id = 3;
        message.putInt(length);
    ...
    public void receiveMessage(SocketChannel socket, Peer peer) throws IOException, NoSuchAlgorithmException, InterruptedException {
        int count = 0;
        ByteBuffer data1 = ByteBuffer.allocate(LENGTH_SIZE);
        while(data1.hasRemaining()) {
            count+= socket.read(data1);
        }
        if (count != LENGTH_SIZE) throw new EOFException();
